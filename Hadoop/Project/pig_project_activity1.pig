-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/nithya/inputs/episode4.txt' USING PigStorage('\t') AS (name: chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/nithya/inputs/episode5.txt' USING PigStorage('\t') AS (name: chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/nithya/inputs/episode6.txt' USING PigStorage('\t') AS (name: chararray, line:chararray);
...
rmf hdfs:///user/nithya/dialogues_outputs;
-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/nithya/dialogues_outputs' USING PigStorage('\t');